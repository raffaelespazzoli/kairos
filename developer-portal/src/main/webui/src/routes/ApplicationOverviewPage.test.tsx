...
describe('ApplicationOverviewPage', () => {
  ...
  it('shows releases error when releases fetch fails', () => {
    mockReleasesResult = {
      data: null,
      error: {
        error: 'unknown',
  ...
  it('does not show health warning when no healthError', () => {
    ...
    expect(
      screen.queryByText('Health data unavailable — Prometheus may be unreachable'),
  ...
  it('refreshes environments and health when refresh button is clicked', async () => {
    const user = userEvent.setup();
    const refreshEnv = vi.fn();
    ...
    expect(refreshEnv).toHaveBeenCalledTimes(1);
