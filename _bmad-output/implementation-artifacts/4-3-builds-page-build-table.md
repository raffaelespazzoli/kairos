...
- [ ] Task 1: Verify story dependencies and current shell wiring before implementation (AC: #1-#7)
  - [ ] Confirm Story 4.1 and Story 4.2 contracts exist or land in the same implementation branch before UI work depends on them
  - [ ] Reuse the existing Builds route at `developer-portal/src/main/webui/src/routes/ApplicationBuildsPage.tsx` instead of creating a second page
  - [ ] Preserve the existing application-context route/tab/breadcrumb structure already wired for `/teams/:teamId/apps/:appId/builds`

- [ ] Task 2: Add frontend build domain types and API helpers (AC: #2, #3, #4, #5)
  - [ ] Create `developer-portal/src/main/webui/src/types/build.ts` with strict TypeScript types for `BuildStatus`, `BuildSummary`, and `BuildDetail`
  - [ ] Create `developer-portal/src/main/webui/src/api/builds.ts` with `fetchBuilds()`, `triggerBuild()`, `fetchBuildDetail()`, and `fetchBuildLogs()`
  - [ ] Keep JSON calls on the shared `apiFetch()` path and handle log retrieval as `text/plain` without bypassing the shared auth/error conventions
  - [ ] Model optional fields correctly (`completedAt`, `duration`, `imageReference`, `failedStageName`, `errorSummary`, `currentStage`, `tektonDeepLink`)

- [ ] Task 3: Implement builds hooks using existing SPA fetch patterns (AC: #1, #3, #4, #6)
  - [ ] Add `developer-portal/src/main/webui/src/hooks/useBuilds.ts` returning `{ data, error, isLoading, refresh }`
  - [ ] Add lazy detail/log loading helpers or hooks for failed-row expansion that remain StrictMode-safe
  - [ ] Use `AbortController` and cleanup guards for all effect-driven fetch logic
  - [ ] Do not add client-side polling, background intervals, or cross-navigation caching

- [ ] Task 4: Add reusable build UI components under `components/build/` (AC: #2, #4, #5, #7)
  - [ ] Create a build status badge/label component that maps portal statuses to PatternFly labels/icons and accessible text
  - [ ] Create a compact build table component with expandable failed rows
  - [ ] Create a failed-build detail panel that renders failed stage, error summary, lazy-loaded logs, and the Tekton deep link
  - [ ] Render artifact references in copy-friendly monospace presentation
  - [ ] Render the "Create Release" button only for passed builds; keep it visibly present for Story 4.4 handoff, but do not implement release POST behavior in this story

- [ ] Task 5: Replace the current Builds page stub with the real page flow (AC: #1, #3, #4, #5, #6, #7)
  - [ ] Build the page header with title, primary "Trigger Build" action, and the shared refresh affordance
  - [ ] Load builds on mount and refresh on demand
  - [ ] On successful trigger, insert or refresh the newest returned build summary at the top of the table without navigating away
  - [ ] Show PatternFly `Spinner`/shared loading UI during fetches
  - [ ] Show inline `Alert`-style error handling for trigger/list/detail/log failures
  - [ ] Implement the specified empty state with Trigger Build and DevSpaces actions

- [ ] Task 6: Keep surrounding application UX coherent (AC: #1, #6)
  - [ ] Ensure the Builds tab remains selected in application navigation when viewing the page
  - [ ] Ensure breadcrumb/view labeling still resolves to "Builds"
  - [ ] If a small follow-up adjustment is needed on the overview's "Recent Builds" placeholder, keep it minimal and aligned with Epic 4 scope

- [ ] Task 7: Verify backend contract alignment and make only minimal supporting changes if required (AC: #2, #3, #4, #5)
  - [ ] Confirm the UI consumes the Story 4.1/4.2 DTO shapes and endpoint semantics as documented
  - [ ] If there is a mismatch between the ready-for-dev stories and the actual implementation branch, fix the smallest contract delta needed in the same branch with tests
  - [ ] Do not add new persistence, polling infrastructure, or alternate build APIs

- [ ] Task 8: Add focused frontend and backend tests for the page workflow (AC: #1-#7)
  - [ ] Add `ApplicationBuildsPage.test.tsx` covering loading, success, error, empty, trigger-build success, failed-row expansion, lazy log loading, and keyboard interaction
  - [ ] Update `App.test.tsx` and any existing Builds-tab tests that still expect the "Coming soon" placeholder
  - [ ] Add or extend API/client tests for `text/plain` log handling if client helpers are changed
  - [ ] If backend contract adjustments are required, extend the corresponding Quarkus tests (`BuildResourceIT`, service tests, adapter tests) in the same change
